...
// Executado quando o arquivo /sys/kernel/devweather/{led, ldr, threshold, temp, tbmp, hum, pa, altitude} é lido (e.g., cat /sys/kernel/devweather/led)
static ssize_t attr_show(struct kobject *sys_obj, struct kobj_attribute *attr, char *buff);
// Executado quando o arquivo /sys/kernel/devweather/{led, ldr, threshold, temp, tbmp, hum, pa, altitude} é escrito (e.g., echo "100" | sudo tee -a /sys/kernel/devweather/led)
...
// Variáveis para criar os arquivos no /sys/kernel/devweather/{led, ldr, threshold, temp, tbmp, hum, pa, altitude}
...
static struct attribute      *attrs[]       = { &led_attribute.attr, &ldr_attribute.attr, &threshold_attribute.attr, &temp_attribute.attr, &tbmp_attribute.attr, &hum_attribute.attr, &pa_attribute.attr, &altitude_attribute.attr, NULL };
...
static ssize_t attr_store(struct kobject *sys_obj, struct kobj_attribute *attr, const char *buff, size_t count) {
    ...
    if (!strcmp(attr_name, "led"))
        ...
        printk(KERN_ALERT "DevWeather: apenas os arquivos led e threshold podem ser escritos. \n");
        return -EACCES;
    }
    if (ret < 0) {
